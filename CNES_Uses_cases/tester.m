...
% Initialize temp var
tempo_LP1 = zeros(size(Im_LCh,1), size(Im_LCh,2)/2);
tempo_HP1 = zeros(size(Im_LCh,1), size(Im_LCh,2)/2);

%x = fliplr(Im_LCh(: ,2:5));
%y = fliplr(Im_LCh(: ,end-4:end-1));
% compute symmetric on image edges for half filter size
X_sym_col   = [fliplr(Im_LCh(: ,2:5)) , Im_LCh(:,:,1) , fliplr(Im_LCh(: ,end-4:end-1))];

for i=5:2:size(Im_LCh,2)+3 %+3 pour indexer ind en fct de i
...
tempo_LP2 = zeros(size(Im_LCh,1)/2, size(Im_LCh,2));
tempo_HP2 = zeros(size(Im_LCh,1)/2, size(Im_LCh,2));

for i=5:2:size(Im_LCh,1)+3 %+3 pour indexer ind en fct de i
